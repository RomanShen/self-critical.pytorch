...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        print('DataLoader loading h5 file: ', opt.input_fc_dir, opt.input_att_dir,
              opt.input_box_dir, opt.input_label_h5, opt.input_image_h5)
        self.h5_label_file = h5py.File(
            self.opt.input_label_h5, 'r', driver='core')
        self.h5_image_file = h5py.File(
            self.opt.input_image_h5, 'r', driver='core')
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        for i in range(batch_size):
            ...
            tmp_label[:, 1: self.seq_length +
                      1] = self.get_captions(ix, seq_per_img)
            ...
            img_batch[i] = preprocess(torch.from_numpy(
                img.astype('float32') / 255.0)).numpy()
